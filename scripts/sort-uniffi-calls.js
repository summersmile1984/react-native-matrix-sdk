...
function sortUniffiCalls() {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (
      line.includes('uniffi.') &&
      line.includes('.uniffiEnsureInitialized()')
    ) {
      uniffiIndices.push(i);
      // Extract just the call part, preserving original indentation
      const match = line.match(
        /^(\s*)(uniffi\.[^.]+\.uniffiEnsureInitialized\(\))/
      );
      if (match) {
        uniffiCalls.push({
          indentation: match[1],
  ...
  const orderChanged = sortedCalls.some(
    (call, index) => call.originalIndex !== uniffiCalls[index].originalIndex
  );
  ...
  console.log(
    `✅ Sorted ${uniffiCalls.length} uniffiEnsureInitialized calls in ${KOTLIN_FILE_PATH}`
  );
